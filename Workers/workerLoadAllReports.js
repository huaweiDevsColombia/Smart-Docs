self.addEventListener('message', function (e) {
    var allReports = [];
    var promise = get("co_sm_report_getList", e.data.username, "100l", "&start=0&limit=1000&dir=DESC&sort=id");
    promise.then(function (reportsRespond) {
        let reports = JSON.parse(reportsRespond);
        allReports.push(reports);
        if (reports.total > 1000) {
            return get("co_sm_report_getList", e.data.username, "100l", "&start=1000&limit=1000&dir=DESC&sort=id").then(function (reportsRespond2) {
